// Generated by CoffeeScript 1.7.1
var UnreadCountManager;

UnreadCountManager = (function() {
  function UnreadCountManager(socket) {
    this.socket = socket;
    this.unreadCount = 0;
    this.socket.onMessage.subscribe(function(e) {
      this.unreadCount++;
      return this._updateTitle();
    });
    $(window).mousemove((function(_this) {
      return function(e) {
        return _this._resetUnreadCount();
      };
    })(this));
    $('#message').keypress((function(_this) {
      return function(e) {
        return _this._resetUnreadCount();
      };
    })(this));
  }

  UnreadCountManager.prototype._resetUnreadCount = function() {
    this.unreadCount = 0;
    return this._updateTitle();
  };

  UnreadCountManager.prototype._updateTitle = function() {
    var title;
    title = "MockingJay";
    if (this.unreadCount > 0) {
      title = "(" + this.unreadCount + ") " + title;
    }
    return document.title = title;
  };

  return UnreadCountManager;

})();

this.ChatWindow = (function() {
  function ChatWindow(socket) {
    var unreadCountManager;
    this.socket = socket;
    this.unreadCount = 0;
    this.$chatWindow = $('#chat');
    this.chime = document.createElement('audio');
    this.chime.setAttribute('src', 'data:audio/wav;base64,');
    this.chime.load();
    unreadCountManager = new UnreadCountManager(this.socket);
    window.onbeforeunload = (function(_this) {
      return function(e) {
        var msg;
        msg = new EventMessage();
        msg.event = "SIGN_OFF";
        socket.sendMessage(msg);
        socket.close();
        if (!e) {
          e = window.event;
        }
        e.stopPropagation();
        return e.preventDefault();
      };
    })(this);
  }

  ChatWindow.prototype._autoScroll = function() {
    if ($(window).scrollTop() < this.maxScrollHeight) {
      return;
    }
    return this.maxScrollHeight = $(window).scrollTop($(window).height()).scrollTop();
  };

  ChatWindow.prototype.update = function() {
    this._autoScroll();
    if (this.$chatWindow.children().length > 200) {
      this.$chatWindow.children()[0].remove();
      return this.$chatWindow.children()[1].remove();
    }
  };

  ChatWindow.prototype.append = function(msg) {
    var $msg;
    if (!msg) {
      return;
    }
    this.unreadCount++;
    $msg = msg.as_html();
    if ((this.$chatWindow.children().length % 2) === 1) {
      $msg.addClass('uc-odd');
    }
    if (msg.text.indexOf(username) !== -1) {
      this.chime.play();
      $msg.addClass('highlight');
    }
    this.$chatWindow.append($msg);
    msg.post_as_html();
    return this.update();
  };

  return ChatWindow;

})();
